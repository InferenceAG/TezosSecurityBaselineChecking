...
case $1 in
	oxford)
		...
		$TEZOSCLIENT originate contract ordering transferring 6 from deploy running ./compiled/ordering/step_002_cont_1_contract.tz --init "$storage" --burn-cap 0.2 --force >out.tmp 2>&1
		$TEZOSCLIENT transfer 0 from admin to ordering --entrypoint 'default' --arg 'Unit' --burn-cap 0.2 >result.tmp 2>&1
